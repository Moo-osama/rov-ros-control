...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0), error_deriv_(3, 0), filtered_error_deriv_(3, 0)
{
  ...
  // defaults
  node_priv.param<double>("Kp", Kp_, 3.0);
  ...
  node_priv.param<std::string>("topic_from_controller", topic_from_controller_, "pitch_control_effort");
  node_priv.param<std::string>("topic_from_plant", topic_from_plant_, "pitch_state");
  node_priv.param<std::string>("setpoint_topic", setpoint_topic_, "pitch_setpoint");
...
void PidObject::doCalcs()
{
  ...
  if (new_state_or_setpt_)
  {
    ...
    {
      if (direction == 1.0)
      control_msg_.data = -1*control_effort_;
      else
