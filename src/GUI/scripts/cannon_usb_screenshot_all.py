...
def writeImage(current_frame):
    key = cv2.waitKey(1)
    if key == ord('1'):
        cv2.imwrite("image1.jpg", current_frame)
        cv2.imshow("image1", current_frame)
        cv2.waitKey(0)
        cv2.destroyWindow('image1')
        writeImage(current_frame)
    elif key == ord('3'):
        cv2.imwrite("image3.jpg", current_frame)
        cv2.imshow("image3", current_frame)
        cv2.waitKey(0)
        cv2.destroyWindow('image3')
        writeImage(current_frame)
    elif key == ord('l'):
        cv2.imwrite("imageL.jpg", current_frame)
        cv2.imshow("imageL", current_frame)
        cv2.waitKey(0)
        cv2.destroyWindow('imageL')
        writeImage(current_frame)
    elif key == ord('c'):
        calculate1(current_frame)

def calculate1(current_frame):
    im = cv2.imread("image1.jpg")
    pts, final_image = get_points_new(im)
    #cv2.imshow('ImageR1', final_image)
    #print (pts)
    if pts != empty:
        ...
        cv2.waitKey(0)
        calculate3(current_frame)
    else:
        calculate1(current_frame)

def calculate3(current_frame):
    ...
    if pts != empty:
        ...
        cv2.waitKey(0)
        calculateL(current_frame)
    else:
        calculate3(current_frame)

def calculateL(current_frame):
    im = cv2.imread("imageL.jpg")
    pts, final_image = get_points(im)
    if pts != empty:
        length2 = calc_distance((pts[0][0][0],pts[0][0][1]),(pts[0][1][0],pts[0][1][1]))
        length1 = calc_distance((pts[1][0][0],pts[1][0][1]),(pts[1][1][0],pts[1][1][1]))
